...
import json
import re
from librdbms.server.rdbms_base_lib import BaseRDBMSDataTable
...
class PrestoApi(Api):
    def __init__(self, user, interpreter=None):
        ...
        coordinator_host = get_coordinator_host(self.options['app_name'], self.options['resource_manager'])
        self.options['url'] = self.options['url'].format(coordinator_host=coordinator_host)

        print self.options

        if self.cache_key in API_CACHE:
            print('self.cache_key in API_CACHE')
            API_CACHE[self.cache_key].db_url = self.options['url']
            self.db = API_CACHE[self.cache_key]
        elif 'password' in self.options:
            print('self.cache_key not in API_CACHE')
            username = self.options.get('user') or user.username
            self.db = API_CACHE[self.cache_key] = Jdbc(self.options['driver'], self.options['url'], username,
                                                       self.options['password'])

    def create_session(self, lang=None, properties=None):
        global API_CACHE
        props = super(PrestoApi, self).create_session(lang, properties)
        ...
        if self.db is None:
            if 'password' in properties:
                ...
                self.db = API_CACHE[self.cache_key] = Jdbc(self.options['driver'], self.options['url'], user,
    ...
    def download(self, notebook, snippet, format):
        ...
        if curs.description:
            ...
            columns = []
        results = BaseRDBMSDataTable(curs, columns)
        ddb = FixedResult(results)
        file_name = 'download'
        return data_export.download(None, format, ddb, id=snippet['id'], file_name=file_name)
